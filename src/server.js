...
app.use('/api/playerResults', require('./routes/playerResult'));
...
io.on('connection', (socket) => {
    ...
    socket.on('add-player', (user, socketId, pin, cb) => {
        ...
        console.log(pinGames);
        // console.log(user, socketId, pin);
        if (pinGames.includes(pin)) {
            leaderBoardGame.filter((leaderboar) => {
                if (leaderboar.pin === pinGameCurrent) {
                    leaderboardCurrent = leaderboar;
                }
            });
            // gameIdCurrent = leaderboardCurrent.gameId;
    ...
    socket.on('start-game', (newQuiz, game, leaderboard) => {
        ...
        socket
            .to(game?.pin)
            .emit('move-to-game-page', game?._id, game?.pin, leaderboard?._id);
    });
    ...
    //     socket.to(pinGame).emit('host-end-game', playerlist, leaderboardCurrent);
    // });
